        ...
        <ion-footer>
            <div>
                <ion-grid>
                    ...
                    <ion-row v-if="status == 1">
                        <ion-col>
                            <ion-button expand="full" color="primary" @click="acceptReservation()">Accept Reservation</ion-button>
                        </ion-col>
                    </ion-row>
                    <ion-row v-if="status == 1">
                        <ion-col>
                            <ion-button expand="full" color="primary" @click="declineReservation()">Decline Reservation</ion-button>
                        </ion-col>
                    </ion-row>
                    <ion-row v-if="status == 2">
                        <ion-col>
...
<script>
import { IonPage, IonHeader, IonToolbar, IonContent, IonFooter, IonButtons, IonButton, IonBackButton, IonGrid, IonRow, IonCol, IonItem, IonLabel, IonInput, modalController } from "@ionic/vue";
...
export default {
    ...
    methods: {
        ...
        acceptReservation() {
             axios.post(Global.methods.GetURL() + "/store_accept_reservation", {
                orderid: this.orderid,
        ...
        declineReservation() {
             axios.post(Global.methods.GetURL() + "/store_decline_reservation", {
                orderid: this.orderid,
                storeid: this.storeid,
            }).then(res => {
                if (res.data == 1) {
        ...
        addExtraCharges() {
            if (this.type != "") {
                if (this.amount != "") {
                    axios.post(Global.methods.GetURL() + "/store_add_extra_charges", {
                        orderid: this.orderid,
                        title: this.type,
                        amount: this.amount,
                    }).then(res => {
                    ...
                    console.log("amount must not be empty");
                }
            } else {
        ...
        payThruWallet() {
            axios.post(Global.methods.GetURL() + "/store_pay_thru_wallet", {
                orderid: this.orderid,
                userid: this.userid,
                storeid: this.storeid,
                ...
                if (res.data == 1) {
                    ...
                    console.log("The customer has insufficient fund.");
                }
            }).catch(err => {
                console.log(err);
            });
